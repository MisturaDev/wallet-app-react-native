...
export default function SendMoneyScreen({ navigation }) {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ title: 'Send Money' });
  }, [navigation]);

  const handleSendMoney = () => {
    const amt = parseFloat(amount);

    if (!recipient || !amount || amt <= 0) {
      Alert.alert('Error', 'Please fill in valid recipient and amount');
      return;
    }
    ...
    Alert.alert('Success', `Sent ${formatNaira(amount)} to ${recipient}`, [
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.background }}>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.paymentContainer}>
          {['Wallet', 'Card', 'Bank Transfer'].map((method) => (
            <TouchableOpacity
              key={method}
...
const styles = StyleSheet.create({
  ...
  sendButton: { backgroundColor: COLORS.primary, paddingVertical: 16, borderRadius: SIZES.radius, alignItems: 'center', marginBottom: 20 },
  sendButtonText: { color: '#fff', fontWeight: 'bold', fontSize: SIZES.font },
});
